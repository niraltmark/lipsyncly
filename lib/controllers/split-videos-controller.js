...
// Openning multiple connections to transloadit doesnt work :(
// We will need to run in synchoronacly
...
//                var filesUrls = docs.map(x => x.FileUrl);
        ...
        var splitDelegates = [0,1,2,3,4,5,6,7].map(function(x) {
            return function(fulfill, reject) {
                transloadit
                    ...
                    .then(function(assmeblyId){
                        var upload = new UploadsModel({ AssemblyId: assmeblyId, GroupId: request.query.groupId, Status: "Splitting" });
         ...
         Promise.sequence(splitDelegates).then(function() {
