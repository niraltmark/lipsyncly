...
//    console.log("task_id:", nodeHelper.task_id);
    ...
    UploadsModel.find({ 'AssemblyId': 'b04283f0e87e11e4ad93c1cbaeff53de'/*nodeHelper.params.AssemblyId*/ }).exec(function(err, upload) { 
        ...
        var splitDuration = 3000; // Eventually we will need to change the duration according to the number of participants, for example if the duration is 3000 and the number of participants is 2 than the final duration will be 6000, and start point will be changed accordingly.
        var splits = Math.floor(duration / splitDuration); // TODO : This definitely a bug that we will need to test for edge cases, need to use the % when dividing 
        
        var iron_worker = require('iron_worker');
        var worker = new iron_worker.Client({"token":"d3mvmf9eVYSmDd7TxfrdX5x4nhc","project_id":"5537658df9335500060000c7"});
        
        var startPointToSplit = startPoint; // The first start point is the start point of the movie
