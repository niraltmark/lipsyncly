...
streamHandlers.download(fileUrl, filename).then(function() {
	...
	// We can speak with transloadit about adding silence detect as a robot, or trim silence as a robot
	ffmpeg.detectSilence(filename).then(function(duration) {
		// Once we will fix the issue with transloadit we can move the code from here
		// This means that we won't need to upload the mute file to transloadit, we will mute the file there
		ffmpeg.mute(filename, "mute_" + filename).then(function() {
			...
			var assemblyOptions = {
				...
				fields: {
					...
					groupId: nodeHelper.params.groupId
				}
			};
